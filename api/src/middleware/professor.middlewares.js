...
const ValidRegisteredProfessors = async (req, res, next) => {
  try {
    ...
    if (!professors || professors.length === 0) {
      return res
        .status(400)
        .send({ message: "Não há professores cadastrados" });
    }
  ...
  try {
    ...
    if (!checkCourseExistence(coursesId)) {
      return res
        ...
        .send({
          message:
            "Todos cursos precisam existir na base de dados, algum coursesId inválido",
        });
    }
...
const ValidUpdate = async (req, res, next) => {
  try {
    ...
    if (
      ...
      res
        .status(400)
        .send({ message: "Pelo menos 1 campo precisa ser atualizado" });
    }
